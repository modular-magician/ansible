...
def update_fields(module, request, response):
    if response.get('quicOverride') != request.get('quicOverride'):
        quicoverride_update(module, request, response)
    if response.get('sslCertificates') != request.get('sslCertificates'):
        sslcertificates_update(module, request, response)
    if response.get('urlMap') != request.get('urlMap'):
        urlmap_update(module, request, response)


def quicoverride_update(module, request, response):
    auth = GcpSession(module, 'compute')
    auth.post(
        ''.join([
...
def sslcertificates_update(module, request, response):
    auth = GcpSession(module, 'compute')
    auth.post(
        ''.join([
            "https://www.googleapis.com/compute/v1/",
...
def urlmap_update(module, request, response):
    auth = GcpSession(module, 'compute')
    auth.post(
        ''.join([
            "https://www.googleapis.com/compute/v1/",
