...
options:
  ...
  initial_node_count:
    ...
    - The number of nodes to create in this cluster. You must ensure that your Compute
      ...
      name. Do not use this and a nodePool at the same time. This field has been deprecated.
      Please use nodePool.initial_node_count instead.
    required: false
  node_config:
    description:
    - Parameters used in creating the cluster's nodes. For requests, this field should
      only be used in lieu of a "nodePool" object, since this configuration (along
      with the "initialNodeCount") will be used to create a "NodePool" object with
      an auto-generated name. Do not use this and a nodePool at the same time. For
      responses, this field will be populated with the node configuration of the first
    ...
    suboptions:
      ...
      metadata:
        ...
        - 'The metadata key/value pairs assigned to instances in the cluster. Keys
          ...
          than 512 KB. An object containing a list of "key": value pairs.'
      ...
      local_ssd_count:
        ...
        - 'The number of local SSD disks to be attached to the node. The limit for
          ...
          machine per zone. See: U(https://cloud.google.com/compute/docs/disks/local-ssd#local_ssd_limits)
...
nodeConfig:
  description:
  - Parameters used in creating the cluster's nodes. For requests, this field should
    only be used in lieu of a "nodePool" object, since this configuration (along with
    the "initialNodeCount") will be used to create a "NodePool" object with an auto-generated
    name. Do not use this and a nodePool at the same time. For responses, this field
    will be populated with the node configuration of the first node pool. If unspecified,
  ...
  contains:
    ...
    oauthScopes:
      ...
      - U(https://www.googleapis.com/auth/devstorage.read_only) is required for communicating
        with gcr.io (the Google Container Registry). If unspecified, no scopes are
        added, unless Cloud Logging or Cloud Monitoring are enabled, in which case
    ...
    metadata:
      ...
      - 'The metadata key/value pairs assigned to instances in the cluster. Keys must
        ...
        KB. The total size of all keys and values must be less than 512 KB. An object
    ...
    localSsdCount:
      ...
      - 'The number of local SSD disks to be attached to the node. The limit for this
        ...
        per zone. See: U(https://cloud.google.com/compute/docs/disks/local-ssd#local_ssd_limits)
        for more information.'
      returned: success
      type: int
    tags:
...
endpoint:
  ...
  - The IP address of this cluster's master endpoint. The endpoint can be accessed
    from the internet at https://username:password@endpoint/ See the masterAuth property
