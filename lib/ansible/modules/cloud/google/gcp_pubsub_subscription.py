...
options:
    ...
    suboptions:
      ...
      attributes:
        ...
        - 'Endpoint configuration attributes. Every endpoint has a set of API supported
          ...
          is based on the version of the Pub/Sub API. If not present during the subscriptions.create
          call, it will default to the version of the API used to make such call.
          If not present during a subscriptions.modifyPushConfig call, its value will
          not be changed. subscriptions.get calls will always return a valid version,
          even if the subscription was created without this attribute. The possible
          values for this attribute are: - v1beta1: uses the push format defined in
          the v1beta1 Pub/Sub API.'
  ...
  message_retention_duration:
    ...
    - 'How long to retain unacknowledged messages in the subscription''s backlog,
      ...
      be more than 7 days (`"604800s"`) or less than 10 minutes (`"600s"`). A duration
  ...
  contains:
    ...
    attributes:
      ...
      - 'Endpoint configuration attributes. Every endpoint has a set of API supported
        ...
        the version of the Pub/Sub API. If not present during the subscriptions.create
        call, it will default to the version of the API used to make such call. If
        not present during a subscriptions.modifyPushConfig call, its value will not
        be changed. subscriptions.get calls will always return a valid version, even
        if the subscription was created without this attribute. The possible values
        for this attribute are: - v1beta1: uses the push format defined in the v1beta1
        Pub/Sub API.'
      - "- v1 or v1beta2: uses the push format defined in the v1 Pub/Sub API."
      returned: success
      type: dict
ackDeadlineSeconds:
  ...
  - This value is the maximum time after a subscriber receives a message before the
    ...
    (10 minutes).
  - If this parameter is 0, a default value of 10 seconds is used. For push delivery,
    this value is also used to set the request timeout for the call to the push endpoint.
...
messageRetentionDuration:
  ...
  - 'How long to retain unacknowledged messages in the subscription''s backlog, from
    ...
    than 7 days (`"604800s"`) or less than 10 minutes (`"600s"`). A duration in seconds
    with up to nine fractional digits, terminated by ''s''. Example: `"600.5s"`.'
  returned: success
  type: str
retainAckedMessages:
  description:
